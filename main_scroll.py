...
def generate_world(worldcode=None, biome=None, screens=5):
    ...
    if worldcode is None:
        ...
        # ground
        noise = g.noise.linear(bio.heights.get(biome, 4) * 2.5, hl, bio.flatnesses.get(biome, 0))
        for _ in range(10):
            g.w.data.append(["soil_f"] * 100)
        g.w.data = SmartList()
        for _ in range(50):
            g.w.data.append(["air"] * 10)
        for _ in range(50):
            g.w.data.append(["dirt"] * 10)
    else:
    ...
    def __init__(self):
        ...
        self.menu_widgets = {
            ...
            "sliders": SmartList([
                ...
                Slider(Window.display,   "Lag",           range(1, 51), 50,                           tooltip="The lag of the camera that fixated on the player", **_menu_slider_kwargs),
...
class Block(Scrollable):
    ...
    def draw(self):  # block draw
        ...
        if is_hard(self.name):
            g.player.block_rects.append(self._rect)
...
class Block(Scrollable):
    def __init__(self, x, y):
        self._rect = pygame.Rect(x * BS, y * BS, BS, BS)
        self.name = g.w.data[y][x]

    def update(self):
        if self.name != "air":
            Window.display.blit(g.w.blocks[self.name], self.rect)
            g.player.block_rects.append(self._rect)
    ...
    with nullcontext() if debug else redirect_stdout(open(os.devnull, "w")):
        ...
        while running:
            ...
            if g.stage == "play":
                ...
                """
                g.player.block_rects = []
                for y, row in enumerate(g.w.data):
                    for x, name in enumerate(row):
                        if name != "air":
                            _rect = pygame.Rect(x * BS, y * BS, BS, BS)
                            rect = [_rect.x - g.scroll[0], _rect.y - g.scroll[1]]
                            if 0 <= rect[0] + BS <= Window.width + BS and 0 <= rect[1] + BS <= Window.height + BS:
                                Window.display.blit(g.w.blocks["soil_f"], rect)
                            if is_hard(name):
                                g.player.block_rects.append(_rect)
